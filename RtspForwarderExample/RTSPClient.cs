...
namespace RtspForwarderExample
{
    internal class RTSPClient
    {
        ...
        public delegate void Received_NALs_Delegate(uint timestamp_ms, List<byte[]> nal_units); // H264 or H265
        ...
        public void Connect(string url, RTP_TRANSPORT rtpTransport, MEDIA_REQUEST mediaRequest = MEDIA_REQUEST.VIDEO_AND_AUDIO)
        {
            ...
            {
                videoUdpPair = new UDPSocket(50000, 51000, _logger); // give a range of 500 pairs (1000 addresses) to try incase some address are in use
                videoUdpPair.DataReceived += RtpDataReceived;
                videoUdpPair.Start(); // start listening for data on the UDP ports
                audioUdpPair = new UDPSocket(50000, 51000, _logger); // give a range of 500 pairs (1000 addresses) to try incase some address are in use
        ...
        public void RtpDataReceived(object? sender, RtspChunkEventArgs e)
        {
            ...
            if (dataReceived.Channel == video_rtcp_channel || dataReceived.Channel == audio_rtcp_channel)
            {
                ...
                // There can be multiple RTCP packets transmitted together. Loop over each one
                long packetIndex = 0;
                while (packetIndex < dataReceived.Data.Length)
                {
                    ...
                    if (rtcp_packet_type == 200)
                    {
                        ...
                        uint ntp_msw_seconds = (uint)(dataReceived.Data[packetIndex + 8] << 24) + (uint)(dataReceived.Data[packetIndex + 9] << 16)
                        + (uint)(dataReceived.Data[packetIndex + 10] << 8) + (uint)(dataReceived.Data[packetIndex + 11]);

                        uint ntp_lsw_fractions = (uint)(dataReceived.Data[packetIndex + 12] << 24) + (uint)(dataReceived.Data[packetIndex + 13] << 16)
                        + (uint)(dataReceived.Data[packetIndex + 14] << 8) + (uint)(dataReceived.Data[packetIndex + 15]);

                        uint rtp_timestamp = (uint)(dataReceived.Data[packetIndex + 16] << 24) + (uint)(dataReceived.Data[packetIndex + 17] << 16)
            ...
            if (dataReceived.Channel == video_data_channel || dataReceived.Channel == audio_data_channel)
            {
                         ...
                         && videoPayloadProcessor is H264Payload)
                {
                    ...
                    else
                    {
                        ...
                        ReceivedNALs?.Invoke(rtp_timestamp / 90, nal_units);
                    }
                }
                else if (dataReceived.Channel == video_data_channel
                         && rtp_payload_type == video_payload
            ...
            if (message.OriginalRequest != null && message.OriginalRequest is Rtsp.Messages.RtspRequestSetup)
            {
                    ...
                    && message.Headers[RtspHeaderNames.Transport] != null)
                {
                    ...
                    if (transport.IsMulticast)
                    {
                        ...
                        videoUdpPair = new UDPSocket(multicast_address, video_data_channel, multicast_address, video_rtcp_channel, _logger);
                        videoUdpPair.DataReceived += RtpDataReceived;
                        videoUdpPair.Start();
        ...
        // MD5 (lower case)
        public static string CalculateMD5Hash(MD5 md5_session, string input)
        {
